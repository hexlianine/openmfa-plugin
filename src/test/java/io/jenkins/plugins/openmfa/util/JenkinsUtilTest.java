...
class JenkinsUtilTest {
  ...
  void testGetJenkinsReturnsInstance(JenkinsRule j) {
    ...
    assertSame(
      j.getInstance(), jenkins.get(), "Should return the same Jenkins instance"
    );
  }
  ...
  void testGetJenkinsVersionFormat(JenkinsRule j) {
    ...
    assertTrue(
      versionString.matches(".*\\d+.*"), "Version should contain at least one digit"
    );
  }
  ...
  void testJenkinsInstanceIsNotNull(JenkinsRule j) {
    ...
    jenkins.ifPresent(instance -> {
      assertNotNull(
        instance.getRootDir(), "Jenkins root directory should not be null"
      );
  ...
  void testJenkinsRootUrlIgnorePathsConstantExists() {
    ...
    assertDoesNotThrow(
      JenkinsUtil::getJenkins, "JenkinsUtil class should load without errors"
    );
  }
